...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function processFileV35(file) {
    ...
    const img = new Image();
    // img.crossOrigin = 'Anonymous';  // eliminado: causa error silencioso con archivos locales
    ...
    img.onload = () => {
      ...
      if (!canvas) { alert('❌ No se encontró tu-canvas'); return; }
      canvas.width  = SIZE;
      canvas.height = SIZE;
    ...
    img.onerror = () => {
      alert('❌ Error al cargar la imagen. Revisa el formato.');

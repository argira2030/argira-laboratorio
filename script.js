...
function sonicParamsV35(fractal_D, colorM, spatialM) {
  ...
  return {
    ...
    _hue_entropy_norm:      hue_entropy_norm,
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // ── Procesar imagen (FileReader — evita problemas con archivos locales)
  function processFileV35(file) {
    ...
    reader.onload = (e) => {
      ...
      img.onload = () => {
        if (!canvas) { alert('❌ No se encontró el canvas'); return; }
        canvas.width  = SIZE;
        canvas.height = SIZE;
        const ctx2d = canvas.getContext('2d');
        ctx2d.drawImage(img, 0, 0, SIZE, SIZE);
        let imgData;
        try { imgData = ctx2d.getImageData(0, 0, SIZE, SIZE); }
        catch(e) { alert('❌ getImageData: ' + e.message); return; }

        let fractal_D, colorM, spatialM, sonic;
        try {
          fractal_D = boxCountingDimensionV35(imgData, SIZE, SIZE);
          colorM    = analyzeColorV35(imgData, SIZE, SIZE);
          spatialM  = analyzeSpatialV35(imgData, SIZE, SIZE);
      ...
      img.onerror = () => {
        alert('❌ No se pudo procesar la imagen. Prueba con JPG o PNG.');
    ...
    reader.onerror = () => {
      alert('❌ Error al leer el archivo. ¿Está dañado?');
